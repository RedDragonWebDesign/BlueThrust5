...
if ( ! empty($_POST['submit']) ) {
	...
	if ($countErrors == 0) {
		...
		$arrColumns = ["question", "accesstype", "multivote", "displayvoters", "resultvisibility", "maxvotes", "pollend", "lastedit_date", "lastedit_memberid"];
		$arrValues = [$_POST['pollquestion'], $_POST['accesstype'], $_POST['multivote'], $_POST['displayvoters'], $_POST['resultvisibility'], $_POST['maxvotes'], $setEndDate, time(), $memberInfo['member_id']];
...
if ( empty($_POST['submit']) ) {
	...
	if ($dispError != "") {
		...
		foreach ($pollObj->getPollOptions() as $pollOptionID) {
			...
			$_SESSION['btPollOptionCache'][$pollOptionCacheID][] = ["value" => $pollOptionInfo['optionvalue'], "color" => $pollOptionInfo['color'], "polloption_id" => $pollOptionInfo['polloption_id']];
		}
